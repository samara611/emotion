...
function validateForm() {
    ...
    if (confirmPassword.value.trim() === '') {
        reset()
        passError.style.display="flex"
        passError.innerHTML='Confirm Password is required' 
        return false; 
    }
    if (confirmPassword.value.length <= 6) {
        reset()
        passError.style.display="flex"
        passError.innerHTML='ConfirmPassword must be at least 6 characters';
    ...
    if (password.value.length!=confirmPassword.value.length) {
        reset()
        passError.style.display="flex"
        passError.innerHTML='Password doesnt Match !';
        return false; // Prevent form submission
    }
    return true;
}
